...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (gamma, x, eps, beta, batch_mean, batch_var, x_norm)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    gamma, x, eps, beta, batch_mean, batch_var, x_norm = cache
    
    d_mult = gamma*dout
    # xi - u_b
    d_num = np.sum((x - batch_mean)*d_mult, axis=0)
    # sqrt(var_b + eps)
    d_den = d_mult * ((batch_var + eps)**-.5)
    # d sqrt(var_b + eps)
    d_den_sqrt = -.5*((batch_var + eps)**-1.5)*d_num
    # d var_b (1 / m * sum ( x - u_b)^2)
    d_den_var_a = d_den_sqrt * np.ones(x.shape) / x.shape[0] 
    # d squared term 
    d_den_var_squared = 2 * (x - batch_mean) * d_den_var_a
    # add both sources of dout1 and dout2
    d_compiled = d_den_var_squared + d_den 
    # -1 accounts for subtraction
    d_mean = -1 * np.sum(d_compiled, axis=0)
    # summation for mean 
    d_mean_compiled = d_mean*np.ones(x.shape) / x.shape[0]
    # compile all sources of dout 
    dx = d_mean_compiled + d_compiled 
...
def conv_backward_naive(dout, cache):
    ...
    for i in xrange(H_prime):
        for j in xrange(W_prime):
            ...
            dx_pad[:,:,i*stride : i*stride+HH, j*stride : j*stride+WW] += np.einsum('ij,jklm->iklm', dout[:,:,i,j], w)
