...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dfj[range(N),y] -= 1
    dfj /= N
    # back prop through first layer 
    grads['W2'] = relu.T.dot(dfj) + 2*reg*W2
    grads['b2'] = np.sum(dfj, axis=0)
    # back prop through reLu layer
    dhid = dfj.dot(W2.T)
    dhid[relu <= 0] = 0
    # back prop through second layer 
    grads['W1'] = X.T.dot(dhid) + 2*reg*W1
  ...
  def predict(self, X):
    ...
    ###########################################################################
    # help from Shayne during OH to debug predict function. Turns out you can use
    # the self loss function to generate the scores
    scores = self.loss(X)
